    ...
    func setMessage(_ message: SBDBaseMessage) {
        ...
        if let sender = (self.msg as? SBDFileMessage)?.sender {
            ...
            if sender.nickname?.count == 0 {
                ...
                self.nicknameLabel.text = sender.nickname
                firstChar = String(sender.nickname?.first ?? "A")
            }
            
            self.profileImageView.setTitle(firstChar, for: .normal)
            if sender.userId == SBDMain.getCurrentUser()!.userId {
                self.nicknameLabel.textColor = .orange
                 self.profileImageView.backgroundColor = .orange
                
            }
            else {
                let greenColor = UIColor.init(red: 43, green: 81, blue: 46)
               self.nicknameLabel.textColor = greenColor
                self.profileImageView.backgroundColor = greenColor
            }
        }        
    }
